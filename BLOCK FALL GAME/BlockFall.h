...
public:
    ...
    void parse_block_shape(const string& line, Block& block);
    vector<vector<bool>> rotate_clockwise(const vector<vector<bool>>& original);
    vector<vector<bool>> rotate_counter_clockwise(const vector<vector<bool>>& original);
    void print_all_blocks(const vector<vector<vector<bool>>>& all_blocks) const;
    vector<bool> parseLine(const string& line);
